...
2. Update `version` and `releaseNotes` in `startos/versions/current.ts` in place — don't rename the file or touch `index.ts`. A _new_ version file is only spun off when the bump requires a migration (git history of `current.ts` preserves the prior release notes automatically) — see [Versions](https://docs.start9.com/packaging/versions.html).
