...
This repo packages [Open WebUI](https://github.com/open-webui/open-webui) for StartOS.
...
Open WebUI runs the upstream `ghcr.io/open-webui/open-webui` image directly — there is no Dockerfile in this repo. To track a new upstream release:

1. Bump `dockerTag` in `startos/manifest/index.ts` to `ghcr.io/open-webui/open-webui:<new version>`.
