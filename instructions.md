...
4. (Optional) To enable web search, install **SearXNG**, then in Open WebUI open **Settings → Web Search** (under the **Admin** section) and turn web search on. The engine and query URL are filled in for you. If you had already been running Open WebUI, restart it once after installing SearXNG so it picks the address up.
