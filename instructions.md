...
- Optional self-hosted web search via [SearXNG](https://github.com/Start9Labs/searxng-startos), wired up for you so Open WebUI's web-search toggle works without entering a URL — whichever of the two you installed first.
...
4. (Optional) To enable web search, install **SearXNG**, then in Open WebUI open **Settings → Web Search** (under the **Admin** section) and turn web search on. The engine and query URL are filled in for you.
...
- **Reconnect SearXNG** — restore web search's connection to SearXNG. Open WebUI normally keeps that address correct for you, but if it gets changed by hand in **Settings → Web Search** it stops doing so, and web search breaks with no way to fix it from the interface. This puts the right address back and resumes maintaining it. Open WebUI restarts; nothing else you've configured is touched. You don't have to go looking for it — if the address is one Open WebUI can no longer manage, StartOS prompts you to run this. (Leave the Searxng Query URL field alone and you'll never need it at all.)
