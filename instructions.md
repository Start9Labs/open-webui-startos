...
3. Run the **Configure Backends** action:
   - Under **Connect detected services**, check the StartOS AI services you want to use. Their internal URL is filled in automatically, along with the API key wherever Open WebUI can read it (Ollama needs none; vLLM and llama.cpp publish theirs). Maple Proxy gets a placeholder key — if you've set a Maple API key, replace it in Open WebUI afterwards.
